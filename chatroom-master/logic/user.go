...
// SendMessage 发送消息给单个用户
func (u *User) SendMessage(ctx context.Context) {
	//将用户的消息通道内的msg写入通向用户的conn通道
	//for-range channel 不会自动退出,需手动关闭MessageChannel
...
// ReceiveMessage 将从用户接收到的消息广播到聊天室
func (u *User) ReceiveMessage(ctx context.Context) error {
	...
	for {
		...
		if err != nil {
			...
			//As()从err树中用根与closeErr对比,相等则true
		...
		// sendMsg初始化为接收到的消息
		sendMsg := NewMessage(u, receiveMsg["content"], receiveMsg["send_time"]) //为什么是"send_time"?
		//过滤敏感词
		sendMsg.Content = FilterSensitive(sendMsg.Content)

		// 制作reg正则查找器,解析 content，看看 @ 谁了,存到Ats切片中
